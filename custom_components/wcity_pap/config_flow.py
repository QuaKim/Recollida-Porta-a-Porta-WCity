...
async def async_validate_input(data: dict) -> dict:
    """Valida las credenciales realizando el proceso de login completo."""
    # Añadimos el token como parámetro en la URL tal como exige el endpoint
    ...
    async with aiohttp.ClientSession(headers=headers, cookie_jar=jar) as session:
        # Paso 1: Obtener cookies iniciales visitando la web
        ...
        # Paso 2: Realizar el login con el token en la URL y el payload
        payload = {
    ...
    return {"title": f"Basura ({data[CONF_USERNAME]})"}
